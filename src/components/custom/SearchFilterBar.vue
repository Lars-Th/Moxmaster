  ...
  <div class="flex flex-col gap-2">
    <div class="flex gap-2 items-center">
      ...
      <DropdownMenu v-if="filterOptions.length > 0">
        ...
        <DropdownMenuContent>
          <DropdownMenuItem @click="updateStatusFilter('all')">Alla</DropdownMenuItem>
          <DropdownMenuItem 
    ...
    <div v-if="showActiveFilters && statusFilter !== 'all'" class="flex gap-2 text-xs">
      ...
      <span class="bg-primary/10 text-primary px-2 py-1 rounded-md">
        ...
        <button @click="clearFilter" class="ml-1 hover:text-primary/70">×</button>
      </span>
    </div>
  </div>
</template> 